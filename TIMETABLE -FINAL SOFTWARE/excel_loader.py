...
import math
from config import (
    INPUT_DIR, REQUIRED_FILES, OPTIONAL_FILES, DEPARTMENTS, MINOR_SUBJECT,
    PRE_MID, POST_MID, SESSION_COLUMN_NAMES, SESSION_VALUES,
    FACULTY_CSE_A_COLUMNS, FACULTY_CSE_B_COLUMNS,
)
...
class ExcelLoader:
    ...
    @staticmethod
    def _parse_instructor_cse_ab(val):
        """Parse combined Instructor format 'CSE-A:Name1, CSE-B:Name2' or 'SEC-A:Name1,SEC-B:Name2' (or semicolon).
        Returns (name_a, name_b) if format matches, else (None, None). Case-insensitive."""
        if pd.isna(val) or str(val).strip() == '':
            return None, None
        s = str(val).strip()
        name_a, name_b = None, None
        
        # Split by comma, semicolon, slash, ampersand, or pipe
        parts = re.split(r'[,;/\&|]', s)
        for part in parts:
            part = part.strip()
            if not part:
            ...
            # Flexible match for (CSE|SEC|SECTION|LAB|CLASS|PART)-(A|B) followed by :, _, or space
            # Also just A:, B:
            m_a = re.match(r'^\s*(?:(?:cse|sec|section|lab|class|part)\s*[- ]?)?a\s*[:\s_]+\s*(.+)$', part, re.IGNORECASE)
            if m_a:
                name_a = m_a.group(1).strip()
                continue
            
            m_b = re.match(r'^\s*(?:(?:cse|sec|section|lab|class|part)\s*[- ]?)?b\s*[:\s_]+\s*(.+)$', part, re.IGNORECASE)
            if m_b:
                name_b = m_b.group(1).strip()
                continue
    ...
    def get_semester_courses(dfs, semester_id):
        ...
        # Explicit sectioned entries (CSE-A, CSE-B) remain as-is; parse combined Instructor so each section gets single name
        explicit_mask = cse_entries['Department'].isin(['CSE-A', 'CSE-B'])
        explicit_cse = cse_entries[explicit_mask].copy()
        col_instructor_cse = 'Instructor' if 'Instructor' in explicit_cse.columns else ('Faculty' if 'Faculty' in explicit_cse.columns else None)
        if col_instructor_cse:
            for idx in explicit_cse.index:
                dept = str(explicit_cse.loc[idx, 'Department']).strip()
                if dept not in ('CSE-A', 'CSE-B'):
                    continue
                val = explicit_cse.loc[idx, col_instructor_cse]
                if pd.isna(val) or not str(val).strip():
                    continue
                name_a, name_b = ExcelLoader._parse_instructor_cse_ab(val)
                if dept == 'CSE-A' and name_a is not None:
                    explicit_cse.loc[idx, col_instructor_cse] = name_a
                elif dept == 'CSE-B' and name_b is not None:
                    explicit_cse.loc[idx, col_instructor_cse] = name_b
        ...
        if not base_cse.empty:
            ...
            # CSE total 160 = CSE-A 80 + CSE-B 80: set per-section capacity so each gets only 2 labs
            if 'Registered Students' in base_cse.columns:
                for idx in cse_a_assigned.index:
                    try:
                        total = int(float(base_cse.loc[idx, 'Registered Students']))
                        if total > 80:
                            cse_a_assigned.loc[idx, 'Registered Students'] = 80
                    except (ValueError, TypeError):
                        pass
                for idx in cse_b_assigned.index:
                    try:
                        total = int(float(base_cse.loc[idx, 'Registered Students']))
                        if total > 80:
                            cse_b_assigned.loc[idx, 'Registered Students'] = 80
                    except (ValueError, TypeError):
                        pass
            # Faculty constraint: optional "Instructor CSE-A" / "Instructor CSE-B" (or Faculty CSE-A / Faculty CSE-B)
            # so CSE-A and CSE-B can have different faculty for the same course
            col_a_candidates = [c for c in FACULTY_CSE_A_COLUMNS if c in base_cse.columns]
            col_b_candidates = [c for c in FACULTY_CSE_B_COLUMNS if c in base_cse.columns]
            col_instructor = 'Instructor' if 'Instructor' in base_cse.columns else ('Faculty' if 'Faculty' in base_cse.columns else None)
            if col_instructor and (col_a_candidates or col_b_candidates):
                col_a = col_a_candidates[0] if col_a_candidates else None
                col_b = col_b_candidates[0] if col_b_candidates else None
                if col_a and col_b:
                    # Overwrite instructor per section where provided
                    for idx in cse_a_assigned.index:
                        val_a = base_cse.loc[idx, col_a]
                        if pd.notna(val_a) and str(val_a).strip():
                            cse_a_assigned.loc[idx, col_instructor] = val_a
                    for idx in cse_b_assigned.index:
                        val_b = base_cse.loc[idx, col_b]
                        if pd.notna(val_b) and str(val_b).strip():
                            cse_b_assigned.loc[idx, col_instructor] = val_b
                    print(f"  Using per-section faculty: {col_a} / {col_b} for CSE-A and CSE-B")
                elif col_a:
                    for idx in cse_a_assigned.index:
                        val_a = base_cse.loc[idx, col_a]
                        if pd.notna(val_a) and str(val_a).strip():
                            cse_a_assigned.loc[idx, col_instructor] = val_a
                    print(f"  Using per-section faculty: {col_a} for CSE-A")
                elif col_b:
                    for idx in cse_b_assigned.index:
                        val_b = base_cse.loc[idx, col_b]
                        if pd.notna(val_b) and str(val_b).strip():
                            cse_b_assigned.loc[idx, col_instructor] = val_b
                    print(f"  Using per-section faculty: {col_b} for CSE-B")
            elif col_instructor:
                # Fallback: parse combined format "CSE-A:SUNIL PV, CSE-B:SUNIL CK" from main Instructor/Faculty column
                parsed_any = False
                for idx in base_cse.index:
                    val = base_cse.loc[idx, col_instructor]
                    name_a, name_b = ExcelLoader._parse_instructor_cse_ab(val)
                    if name_a is not None or name_b is not None:
                        if name_a is not None:
                            cse_a_assigned.loc[idx, col_instructor] = name_a
                        if name_b is not None:
                            cse_b_assigned.loc[idx, col_instructor] = name_b
                        parsed_any = True
                if parsed_any:
                    print(f"  Using combined faculty format (CSE-A:..., CSE-B:...) – separate lecturers, no overlap")
        ...
        # NOTE: Previously we printed warnings when CSE-A and CSE-B used the same
        # faculty for the same course. This validation was only informational and
        # could be confusing, so it has been disabled to keep the run output clean.
        # The timetable generation itself does not depend on this check.
    ...
    def parse_ltpsc(courses_df):
        ...
        - L: Lecture HOURS per week (converted to lecture sessions: each session is 1.5 hours)
        - T: Tutorial HOURS per week (converted to tutorial sessions: each session is 1 hour)
        ...
        if 'LTPSC' in df.columns:
            ...
            for idx, row in df.iterrows():
                ...
                # L, T, P represent HOURS per week, not sessions per week
                # Convert hours to sessions by dividing by session duration and rounding up
                try:
                    l = float(parts[0])  # Lecture hours per week
                    t = float(parts[1])  # Tutorial hours per week
                    p = float(parts[2])  # Lab hours per week
                    
                    # Convert lecture hours to lecture sessions (each lecture is 1.5 hours)
                    # If l=3 hours, that means 3/1.5 = 2 lecture sessions per week
                    # If l=2 hours, that means 2/1.5 = 1.33, round up to 2 sessions
                    lecture_hours_per_session = 1.5  # Each lecture is 1.5 hours
                    lecture_sessions = math.ceil(l / lecture_hours_per_session) if l > 0 else 0
                    df.at[idx, 'Lectures_Per_Week'] = int(lecture_sessions)
                    
                    # Convert tutorial hours to tutorial sessions (each tutorial is 1 hour)
                    # If t=1 hour, that means 1/1 = 1 tutorial session per week
                    # If t=2 hours, that means 2/1 = 2 tutorial sessions per week
                    tutorial_hours_per_session = 1.0  # Each tutorial is 1 hour
                    tutorial_sessions = math.ceil(t / tutorial_hours_per_session) if t > 0 else 0
                    df.at[idx, 'Tutorials_Per_Week'] = int(tutorial_sessions)
                    
                    # Convert lab hours to lab sessions (each lab session is 2 hours)
                    # If p=2 hours, that means 2/2 = 1 lab session per week
                    # If p=4 hours, that means 4/2 = 2 lab sessions per week
                    lab_hours_per_session = 2.0  # Each lab session is 2 hours
                    lab_sessions = math.ceil(p / lab_hours_per_session) if p > 0 else 0
    ...
    @staticmethod
    def _normalize_session_value(val):
        """Normalize Session column value to Pre-Mid, Post-Mid, Full, or None (blank/invalid)."""
        if pd.isna(val) or str(val).strip() == '':
            return None
        s = str(val).strip().lower()
        for canonical, variants in SESSION_VALUES.items():
            if s in variants or s == canonical.lower():
                return canonical
        return None
    ...
    def divide_courses_by_session(courses_df, department, all_sem_courses=None):
        ...
        RULES:
        - If course_data has Session column (Pre / Post / Full): that value OVERRIDES derived logic.
        - Electives: ALWAYS run FULL semester (included in BOTH Pre-Mid and Post-Mid) unless overridden by Session.
        - Credits > 2: Run FULL semester (included in BOTH Pre-Mid and Post-Mid) unless overridden.
        - Credits <= 2: Run HALF semester (split equally - EITHER Pre-Mid OR Post-Mid) unless overridden.
        ...
        # --- Session column (Pre / Post / Full): explicit override when present ---
        session_col = None
        for colname in SESSION_COLUMN_NAMES:
            if colname in courses_df.columns:
                session_col = colname
                break
        explicit_pre_df = pd.DataFrame()
        explicit_post_df = pd.DataFrame()
        explicit_full_df = pd.DataFrame()
        explicit_session_codes = set()  # Course codes with explicit Pre or Post (do not move in 2-credit sharing)
        if session_col:
            for idx, row in courses_df.iterrows():
                val = ExcelLoader._normalize_session_value(row.get(session_col))
                if val == PRE_MID:
                    explicit_pre_df = pd.concat([explicit_pre_df, row.to_frame().T], ignore_index=True)
                    if 'Course Code' in row.index:
                        explicit_session_codes.add(str(row['Course Code']).strip())
                elif val == POST_MID:
                    explicit_post_df = pd.concat([explicit_post_df, row.to_frame().T], ignore_index=True)
                    if 'Course Code' in row.index:
                        explicit_session_codes.add(str(row['Course Code']).strip())
                elif val == 'Full':
                    explicit_full_df = pd.concat([explicit_full_df, row.to_frame().T], ignore_index=True)
            if not explicit_pre_df.empty or not explicit_post_df.empty or not explicit_full_df.empty:
                print(f"  Session column '{session_col}' used: {len(explicit_pre_df)} Pre-Mid only, {len(explicit_post_df)} Post-Mid only, {len(explicit_full_df)} Full (override)")
        ...
        # --- Apply Session column override (Pre / Post / Full) ---
        if not explicit_pre_df.empty or not explicit_post_df.empty or not explicit_full_df.empty:
            explicit_pre_codes = set(explicit_pre_df['Course Code'].dropna().astype(str)) if 'Course Code' in explicit_pre_df.columns and not explicit_pre_df.empty else set()
            explicit_post_codes = set(explicit_post_df['Course Code'].dropna().astype(str)) if 'Course Code' in explicit_post_df.columns and not explicit_post_df.empty else set()
            # Remove explicit Post from pre_mid; remove explicit Pre from post_mid
            if explicit_post_codes and 'Course Code' in pre_mid_courses.columns and not pre_mid_courses.empty:
                pre_mid_courses = pre_mid_courses[~pre_mid_courses['Course Code'].astype(str).isin(explicit_post_codes)].reset_index(drop=True)
            if explicit_pre_codes and 'Course Code' in post_mid_courses.columns and not post_mid_courses.empty:
                post_mid_courses = post_mid_courses[~post_mid_courses['Course Code'].astype(str).isin(explicit_pre_codes)].reset_index(drop=True)
            # Add explicit Pre and Full to pre_mid; explicit Post and Full to post_mid
            if not explicit_pre_df.empty:
                pre_mid_courses = pd.concat([pre_mid_courses, explicit_pre_df], ignore_index=True)
            if not explicit_full_df.empty:
                pre_mid_courses = pd.concat([pre_mid_courses, explicit_full_df], ignore_index=True)
            if not explicit_post_df.empty:
                post_mid_courses = pd.concat([post_mid_courses, explicit_post_df], ignore_index=True)
            if not explicit_full_df.empty:
                post_mid_courses = pd.concat([post_mid_courses, explicit_full_df], ignore_index=True)
        ...
        # Apply shared 2-credit course coordination across CSE/DSAI/ECE (do not move explicit Pre/Post courses)
        pre_mid_courses, post_mid_courses = ExcelLoader._apply_two_credit_sharing(
            pre_mid_courses,
            post_mid_courses,
            department,
            all_sem_courses,
            explicit_session_codes=explicit_session_codes,
        )
    ...
    @staticmethod
    def _apply_two_credit_sharing(pre_mid_df, post_mid_df, department, all_sem_courses, explicit_session_codes=None):
        ...
        Courses in explicit_session_codes (explicit Pre or Post from Session column) are never moved."""
        ...
        explicit_session_codes = set(explicit_session_codes) if explicit_session_codes is not None else set()
        ...
        # Also never move courses with explicit Pre or Post from Session column.
        ...
        elective_codes_union = pre_elective_codes | post_elective_codes | explicit_session_codes
