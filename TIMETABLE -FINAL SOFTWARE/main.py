...
class TimetableGenerator:
    ...
    def setup_environment(self):
        ...
        self.exam_scheduler = ExamScheduler(self.data_frames, self.schedule_generator)
    ...
    def generate_timetables(self, semesters=None):
        ...
        # Before exporting classroom views, resolve any room clashes by reallocating
        try:
            print("\nResolving classroom room conflicts (if any)...")
            fixed = self.schedule_generator.resolve_room_conflicts()
            if fixed:
                print(f"Resolved {fixed} room allocation conflicts by moving classes to free rooms.")
            else:
                print("No room allocation conflicts detected.")
        except Exception as e:
            print(f"WARNING: Could not resolve room conflicts automatically: {e}")

        # Generate unified classroom view
        try:
            print("\nGenerating unified classroom view...")
            if self.excel_exporter.export_classroom_view():
                 print("SUCCESS: Classroom view generated.")
            else:
                 print("FAILED: Classroom view generation failed.")
        except Exception as e:
            print(f"ERROR generating classroom view: {e}")

        # Generate unified faculty view
        try:
            print("\nGenerating unified faculty view...")
            if self.excel_exporter.export_faculty_view():
                 print("SUCCESS: Faculty view generated.")
            else:
                 print("FAILED: Faculty view generation failed.")
        except Exception as e:
            print(f"ERROR generating faculty view: {e}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            print("="*80)
            if generator.exam_scheduler.export_exam_timetable():
                print("SUCCESS: Exam timetable generated")
            else:
                print("FAILED: Exam timetable generation failed")
        except Exception as e:
            print(f"ERROR generating exam timetable: {e}")
            import traceback
            traceback.print_exc()
